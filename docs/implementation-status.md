...
|---|---|---|---|
| 0 Compatibility spike | Partial | Provider matrix, Codex and Qoder fixtures, macOS/Windows IPC source, live macOS Tauri window/tray verification, and live ESP32-C3 USB hardware verification | Real Claude fixtures and clean Windows host verification |
...
| 6 Devices | Implemented for USB first slice | Desktop traffic light, saved LED effects, Rust-owned USB serial connection, port discovery, state/effect/brightness synchronization, ESP32-C3 firmware, and live GFlash6/minic hardware verification | Desktop BLE transport, automatic reconnect after unplug/replug, and clean Windows hardware verification |
